...
library("e1071")
rm(list=ls())
bank <- read.csv("C:/Users/amuna/OneDrive/Documents/Assignments/assignment 3/UniversalBank.csv ")
bank$Personal.Loan <- as.factor(bank$Personal.Loan)
bank$Online <- as.factor(bank$Online)
bank$CreditCard <- as.factor(bank$CreditCard)
set.seed(64060)
train_set = createDataPartition(bank$Personal.Loan, p=.6, list = F) #60% train
train_df = bank[train_set,]
validation_df = bank[-train_set,]
```
...
```{r pivot}
melt_bank = melt(train_df, id=c("CreditCard","Personal.Loan"), variable = "Online") # elongated DF to reshape id, var into $value 
dcast_bank = dcast(melt_bank, CreditCard+Personal.Loan~Online)
dcast_bank[,c(1:2,14)] # CreditCard, Personal.Loan, Online DF
```
...
cc_melt_bank = melt(train_df, id=c("CreditCard"), variable = "Online")
dcast_loan_bank = dcast(loan_melt_bank, Personal.Loan~Online)
dcast_cc_bank = dcast(cc_melt_bank, CreditCard~Online)
